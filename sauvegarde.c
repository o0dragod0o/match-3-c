...
int sauvegarderPartie(Jeu *p) {
    ...
    for (int i = 0; i < 6; i++) {
        if (i) fprintf(f, " ");
        fprintf(f, "%d", p->objectifs[i]);  // Sauvegarde les objectifs pour chaque type
    }
    fprintf(f, "\n");
    for (int i = 0; i < 6; i++) {
        if (i) fprintf(f, " ");
        fprintf(f, "%d", p->collecte[i]);  // Sauvegarde les items collectés
    }
    fprintf(f, "\n");
    // Sauvegarde la grille complète
...
int chargerPartie(Jeu *p) {
    ...
    // Test version (en-tête "MATCH3SAVE")
    if (strncmp(line, "MATCH3SAVE", 9) == 0) {
        ...
        if (!tok) {
            ...
            p->mursCasses = atoi(tok);
            tok = strtok(NULL, " \t\r\n"); 
            if (!tok) p->objectifMurs = 0;
            else p->objectifMurs = atoi(tok); 
        }
        if (fgets(line, sizeof(line), f)) {
            sscanf(line, "%d %d %d %d %d %d",
                   &p->objectifs[0], &p->objectifs[1], &p->objectifs[2], &p->objectifs[3], &p->objectifs[4], &p->objectifs[5]);
        } else {
            for (int i=0;i<6;i++) p->objectifs[i]=0;
        }
        if (fgets(line, sizeof(line), f)) {
            sscanf(line, "%d %d %d %d %d %d",
                   &p->collecte[0], &p->collecte[1], &p->collecte[2], &p->collecte[3], &p->collecte[4], &p->collecte[5]);
        } else {
            for (int i=0;i<6;i++) p->collecte[i]=0;
        }
        for (int i = 0; i < LIGNES; i++) {
            if (!fgets(line, sizeof(line), f)) {
                for (int j = 0; j < COLONNES; j++) {
                    p->plateau[i][j].type = VIDE; p->plateau[i][j].estSelectionne = 0;
            ...
            for (int j = 0; j < COLONNES; j++) {
                if (tok) {
                    int t = atoi(tok); p->plateau[i][j].type = (TypeItem)t;
                    tok = strtok(NULL, " \t\r\n");
                }
